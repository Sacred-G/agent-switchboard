...
All notable changes to Agent Switchboard will be documented in this file.
...
* complete switchboard fixes and automate releases ([70607f8](https://github.com/Sacred-G/agent-switchboard/commit/70607f8253cd37e699877a6d2eb8adb232dec014))


### Bug Fixes

* install Linux audio build dependency ([a5da5fb](https://github.com/Sacred-G/agent-switchboard/commit/a5da5fbf5279c908777a30a995fdeda0b4950238))
* synchronize Rust release versions ([#10](https://github.com/Sacred-G/agent-switchboard/issues/10)) ([0e64c71](https://github.com/Sacred-G/agent-switchboard/commit/0e64c71cbe333ca42edf3d908c691fb237799032))
...
- **Codex CLI Models Endpoint**: The local proxy now answers `GET /v1/models`, which Codex CLI probes at startup, returning the agent-switchboard-managed Codex model catalog. A stale-catalog guard parses the live `config.toml` and only serves the catalog when `model_catalog_json` still references the agent-switchboard-owned file, so a leftover catalog from a previous provider is not advertised (#3818).
...
- **Codex Chat Truncated Stream Detection**: When a Chat Completions upstream ends a stream without a `finish_reason` or `[DONE]`, Agent Switchboard no longer reports it as a normal completion — it finalizes normally only when the stream truly finished, emits an incomplete (`max_output_tokens`) response when partial output was produced, and emits a failed `stream_truncated` event when nothing was produced. Late-arriving reasoning is also attached to still-active streamed tool calls.
...
- **Codex Model Catalog Path Portability**: Codex now writes only the relative filename `agent-switchboard-model-catalog.json` to `config.toml` instead of an absolute path (Codex CLI resolves it from the config directory), fixing the model catalog breaking on WSL and symlinked setups where the absolute path could not be translated (#3614).
...
- **Windows Tray Icon Residue on Exit**: Quitting Agent Switchboard on Windows could leave a dead tray icon until hovered; the app now removes the tray icon before exiting so it disappears cleanly (#3797).
- **Windows Taskbar Icon**: Set an explicit Windows AppUserModelID at runtime and stamped the installer's desktop and start-menu shortcuts with the same ID and product icon, so Agent Switchboard shows the correct icon and groups properly in the taskbar (#3457).
...
- **Codex Chat Completions routing**: Codex providers can now be served by OpenAI-compatible Chat Completions upstreams. Agent Switchboard converts Codex Responses requests into Chat Completions, rebuilds JSON and SSE responses back into Responses shape, preserves reasoning / `<think>` / tool-call state, normalizes error envelopes, and probes Chat-format providers correctly in Stream Check.
...
- **Codex Chat Completions Routing**: Codex providers can now be served by upstreams that only speak the OpenAI Chat Completions API. Agent Switchboard's local proxy converts Codex's outgoing Responses requests into Chat Completions and rebuilds the Chat response (both JSON and SSE) back into Responses shape, preserving `reasoning_content`, inline `<think>` blocks, streamed reasoning summaries, tool calls, and `previous_response_id` follow-ups. A bounded Codex Chat history cache restores tool calls before their tool outputs.
...
- **Codex Model Mapping Table**: Codex provider forms now expose a model catalog (model + display name + context window per row) that is the single source of truth for the upstream model list, projected to `~/.codex/agent-switchboard-model-catalog.json`.
...
- **Homebrew Official Cask Installation**: Installation simplified to `brew install --cask agent-switchboard` now that Agent Switchboard is in the official Homebrew repository; the personal-tap requirement was removed from all READMEs.
...
- **Claude Desktop Third-Party Provider Switching via Proxy Gateway**: Added a dedicated Claude Desktop surface that brokers third-party Claude providers through Agent Switchboard's in-app proxy, with a routing-support badge for providers that need it, role-based model route mapping locked to `sonnet` / `opus` / `haiku`, Copilot/Codex OAuth provider reuse, a redesigned Claude Code import flow, app-switcher differentiation between "Claude Code" and "Claude Desktop", and 44 provider presets translated from the Claude Code catalog.
...
- **Claude Desktop Operational Notes**: Switching a Claude Desktop provider now writes Agent Switchboard's managed 3P profile and requires restarting Claude Desktop to take effect; proxy-mode providers require Agent Switchboard Local Routing to stay running.
...
- **Agent Switchboard Brand Surface Unified to agent-switchboard.io**: All in-app and README references now point at agent-switchboard.io as the sole official website; the release notes template also surfaces agent-switchboard.io.
...
- **Release Notes Template**: Surfaces `agent-switchboard.io` in the release notes template.
- **Brand Surface**: Documented `agent-switchboard.io` as the sole official website across READMEs and in-app references.
...
- **Hermes Config Health Scanner**: Removed the in-app Hermes config health scanner, warning banner, `scan_hermes_config_health` command, `HermesHealthWarning` type, and `HermesWriteOutcome.warnings` payload. Agent Switchboard now keeps the Hermes surface focused on active provider display, provider switching defaults, memory editing, and launching the Hermes Web UI for deep configuration.
...
- **Codex OAuth Responses SSE Aggregation**: Non-streaming Anthropic clients now receive JSON even when the ChatGPT Codex upstream forces OpenAI Responses SSE; Agent Switchboard aggregates the upstream SSE events before running the non-streaming transform (#2235).
...
- **Hermes Memory Panel**: Added a Memory panel for editing `MEMORY.md` and `USER.md` directly from Agent Switchboard, with an enable switch, character-count limits, and a live save flow. Replaces the Prompts entry for Hermes.
...
- **Application-Level Window Controls**: Added an opt-in setting to render Agent Switchboard's own minimize / toggle-maximize / close buttons instead of the system decorations, materially improving the experience on Linux Wayland where compositor-drawn buttons can become inert.
...
- **OpenClaw Config Directory Override**: Added a settings option to point Agent Switchboard at a custom `openclaw.json` location.
- **Hermes Config Directory Override**: Added a settings option to point Agent Switchboard at a custom `~/.hermes/config.yaml` location, backed by data-driven dispatch.
...
- **Hermes Deep Config Delegation**: Deep YAML knobs are now delegated to the Hermes Web UI via a direct launch action, rather than duplicated in the Agent Switchboard form.
...
- **Tray Menu ID Constant**: The tray identifier moved from the hardcoded string `"main"` to a `TRAY_ID` constant (`"agent-switchboard"`) across all call sites.
...
- **Hermes YAML Pollution & OAuth MCP Auth Drop**: Round-tripping through Agent Switchboard no longer drops OAuth MCP `auth` blocks or pollutes unrelated YAML keys; guard tests added via `tests/hermes_roundtrip.rs`.
...
- **Lightweight Mode**: Added a tray-only mode that destroys the main window and keeps Agent Switchboard running from the system tray, with the window recreated when users reopen it.
...
- **Skills Discovery & Batch Updates**: Added SHA-256 based skill update detection, per-skill and batch update actions, a storage-location toggle between Agent Switchboard and `~/.agents/skills`, and public `skills.sh` search integration.
...
- **Skill Auto-Backup**: Skill files are automatically backed up to `~/.agent-switchboard/skill-backups/` before uninstall, with metadata preserved in `meta.json`; old backups pruned to keep at most 20
...
- **OpenClaw Integration**: Full management support for OpenClaw as the fifth application in Agent Switchboard, including provider switching, configuration panels (Env / Tools / Agents Defaults), Workspace file management (HEARTBEAT / BOOTSTRAP / BOOT), daily memory files, and additive overlay mode
...
- **Crash Logging** - Panic hook captures crash info to `~/.agent-switchboard/crash.log` with full stack traces (#562)
...
- **Windows Title** - Display "Agent Switchboard" instead of default "Tauri app" in window title
...
- **Import/Export** - Restrict SQL import to Agent Switchboard exported backups only; refresh providers immediately after import
...
- **ArchLinux installation support** (#259) - Added AUR installation via `paru -S agent-switchboard-bin`
...
- **Deep link support** - Import Gemini providers via `agent-switchboard://` protocol
...
#### Deep Link Protocol (agent-switchboard://)

- **Protocol registration** - `agent-switchboard://` URL scheme for one-click imports
...
- **Standardized release artifact naming** - All platform releases now use consistent version-tagged filenames:
  - macOS: `Agent-Switchboard-v{version}-macOS.tar.gz` / `.zip`
  - Windows: `Agent-Switchboard-v{version}-Windows.msi` / `-Portable.zip`
  - Linux: `Agent-Switchboard-v{version}-Linux.AppImage` / `.deb`
...
#### Backup on v1→v2 Migration (agent-switchboard internal config)

- When the app detects an old v1 config structure at `~/.agent-switchboard/config.json`, it now creates a timestamped backup before writing the new v2 structure.
- Backup location: `~/.agent-switchboard/config.v1.backup.<timestamp>.json`
- This only concerns agent-switchboard's own metadata file; your actual provider files under `~/.claude/` and `~/.codex/` are untouched.
