...
const UsageScriptModal: React.FC<UsageScriptModalProps> = ({
  ...
  const validateTimeout = (value: string): number => {
    ...
    if (!Number.isInteger(num)) {
      toast.warning(
        t("usageScript.timeoutMustBeInteger") || "Timeout must be an integer",
      );
    }
    if (num < 0) {
      toast.error(
    ...
    if (!Number.isInteger(num)) {
      toast.warning(
        t("usageScript.intervalMustBeInteger") ||
          "Auto query interval must be an integer",
      );
    }
    if (num < 0) {
      toast.error(
        t("usageScript.intervalCannotBeNegative") ||
          "Auto query interval cannot be negative",
      );
      return 0;
    }
    ...
    if (clamped !== num && num > 0) {
      toast.info(
        t("usageScript.intervalAdjusted", { value: clamped }) ||
          `Auto query interval adjusted to ${clamped} minutes`,
      );
    }
    return clamped;
  };
