    ...
    try {
      ...
      toast.error(
        t("proxy.takeover.failed", {
          detail,
          defaultValue: "Failed to toggle routing: {{detail}}",
        }),
      );
    }
  };
    ...
    if (!isValidAddress) {
      toast.error(
        t("proxy.settings.invalidAddress", {
          defaultValue:
            "Invalid address. Enter an IPv4 (e.g. 127.0.0.1), IPv6 (e.g. ::1), or localhost",
        }),
      );
      return;
    }
    ...
    try {
      ...
      toast.success(
        t("proxy.settings.configSaved", {
          defaultValue: "Routing configuration saved",
        }),
        { closeButton: true },
      );
    } catch (error) {
            ...
            <div className="space-y-1">
              <p className="text-sm font-medium leading-none">
                {t("proxyConfig.proxyEnabled", {
                  defaultValue: "Routing Master Switch",
                })}
              </p>
              ...
              <div className="rounded-xl border-2 border-primary/20 bg-primary/5 p-4 space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  {t("proxy.takeover.hint", {
                    defaultValue:
                      "Select apps to route — once enabled, requests from that app will go through local routing",
                  })}
                </p>
              </div>
            </motion.div>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  {t("proxy.settings.restartRequired", {
                    defaultValue:
                      "Restart routing service for address or port changes to take effect",
                  })}
                </p>
              </div>
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("proxy.settings.fields.enableLogging.description", {
                        defaultValue:
                          "Log all routing requests for troubleshooting",
                      })}
                    </p>
                  </div>
                  <Switch
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {t("proxy.settings.basic.description", {
                    defaultValue:
                      "Configure routing service listening address and port.",
                  })}
                </p>
              </div>
            ...
            <div className="text-center py-6 text-muted-foreground">
              ...
              <p className="text-base font-medium text-foreground mb-1">
                {t("proxy.panel.stoppedTitle", {
                  defaultValue: "Routing Service Stopped",
                })}
              </p>
              <p className="text-sm text-muted-foreground">
