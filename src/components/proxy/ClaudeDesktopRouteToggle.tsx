...
export function ClaudeDesktopRouteToggle({
      ...
      if (otherTakeoverActive) {
        toast.warning(
          t("claudeDesktop.route.stopBlockedByTakeover", {
            defaultValue:
              "Other apps are currently using proxy takeover. Please disable takeover for those apps in settings before stopping local routing.",
          }),
          { duration: 5000 },
        );
        return;
  ...
  const tooltipText = isRunning
    ? t("claudeDesktop.route.tooltip.active", {
        ...
        port: routePort,
        defaultValue: `Claude Desktop local routing enabled - ${routeAddress}:${routePort}`,
      })
    : t("claudeDesktop.route.tooltip.inactive", {
        address: routeAddress,
        port: routePort,
        defaultValue: `Enable Claude Desktop local routing for providers requiring model mapping or format conversion. Current configuration address: ${routeAddress}:${routePort}`,
      });
