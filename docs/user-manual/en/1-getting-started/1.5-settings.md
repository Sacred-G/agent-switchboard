...
This section describes how to configure Agent Switchboard according to your preferences.
...
Agent Switchboard supports four languages:
...
When enabled, Agent Switchboard automatically runs when the system starts.
...
Starting from v3.13.0, Agent Switchboard adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger Agent Switchboard via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If Agent Switchboard runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, Agent Switchboard automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that Agent Switchboard uses when opening a terminal.
...
The storage location for Agent Switchboard's own data, defaulting to `~/.agent-switchboard/`.
...
The exported file name format is `agent-switchboard-export-{timestamp}.sql`.
...
Configure Agent Switchboard's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, Agent Switchboard automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `agent-switchboard-sync`) |
...
- Agent Switchboard automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, Agent Switchboard displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current Agent Switchboard version number, with support for:
