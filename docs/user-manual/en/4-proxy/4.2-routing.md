...
App routing means letting Agent Switchboard route a specific application's API requests through the local routing service.
...
When routing is enabled, Agent Switchboard modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, Agent Switchboard will:
...
Before enabling routing, Agent Switchboard backs up the original configuration and restores it when disabled.
