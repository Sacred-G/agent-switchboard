...
## Agent Switchboard Data Storage

### Storage Directory

Default location: `~/.agent-switchboard/`
...
```
~/.agent-switchboard/
├── agent-switchboard.db      # SQLite database (SSOT)
...
`agent-switchboard.db` is a SQLite database that stores:
...
Hermes uses YAML configuration. Agent Switchboard writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to Agent Switchboard's "providers") |
...
Agent Switchboard's priority when modifying configurations:

1. **Agent Switchboard Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by Agent Switchboard)
- Agent Switchboard's `settings.json`

### Not Recommended to Edit Manually

- `agent-switchboard.db` database file
- Backup files
...
Agent Switchboard v3.7.0 migrated from JSON files to SQLite:
