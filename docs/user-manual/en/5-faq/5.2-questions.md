...
Agent Switchboard for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/farion1231/agent-switchboard/releases).
...
2. Add Agent Switchboard to your antivirus software's whitelist
...
chmod +x Agent-Switchboard-*.AppImage

# If it still fails, try
./Agent-Switchboard-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Check if the `~/.agent-switchboard/` directory exists
2. Restore from backup: `~/.agent-switchboard/backups/`
...
**Solutions**:
1. Confirm the file was exported by Agent Switchboard
...
**Windows**:
- Check taskbar settings to ensure the Agent Switchboard icon is not hidden
...
2. Restart the app
3. Delete `~/.agent-switchboard/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask agent-switchboard`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and Agent Switchboard runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, Agent Switchboard covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `agent-switchboard://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/farion1231/agent-switchboard/issues)
2. Search for similar issues
3. If none found, create a new Issue
4. Provide the following information:
   - Operating system and version
   - Agent Switchboard version
...
- macOS/Linux: `~/.agent-switchboard/logs/`
- Windows: `%APPDATA%\agent-switchboard\logs\`
