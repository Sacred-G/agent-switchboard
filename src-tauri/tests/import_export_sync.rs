...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".agent-switchboard").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".agent-switchboard");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not ~/.agent-switchboard/settings.json
...
fn export_sql_returns_error_for_invalid_path() {
    ...
    let invalid_parent = if cfg!(windows) {
        std::env::temp_dir().join("agent-switchboard-test-invalid<>dir")
    } else {
    ...
    match err {
        AppError::IoContext { context, .. } => {
            assert!(
                context.contains("Writefailed") || context.contains("Writefailed"),
...
fn import_sql_rejects_non_cc_switch_backup() {
    ...
    let import_path = home.join("not-agent-switchboard.sql");
    fs::write(&import_path, "CREATE TABLE x (id INTEGER);").expect("write import sql");

    let err = state
        .db
        .import_sql(&import_path)
        .expect_err("non-agent-switchboard sql should be rejected");
...
fn import_sql_accepts_cc_switch_exported_backup() {
    ...
    let state = create_test_state_with_config(&config).expect("create test state");
    let export_path = home.join("agent-switchboard-export.sql");
    state
        .db
        .export_sql(&export_path)
        .expect("export should succeed");
