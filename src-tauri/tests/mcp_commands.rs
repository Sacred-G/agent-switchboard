...
fn import_default_config_claude_persists_provider() {
    ...
    let db_path = home.join(".agent-switchboard").join("agent-switchboard.db");
    assert!(
        db_path.exists(),
        "importing default config should persist to agent-switchboard.db"
    );
}
...
fn import_default_config_without_live_file_returns_error() {
    ...
    match err {
        AppError::Localized { zh, .. } => assert!(
            zh.contains("Claude Code Configure"),
            "unexpected error message: {zh}"
        ),
        AppError::Message(msg) => assert!(
            msg.contains("Claude Code Configure"),
...
fn import_mcp_from_claude_creates_config_and_enables_servers() {
    ...
    assert!(
        db_path.exists(),
        "state.save should persist to agent-switchboard.db when changes detected"
    );
}
