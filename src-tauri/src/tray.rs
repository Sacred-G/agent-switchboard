...
pub const TRAY_ID: &str = "agent-switchboard";
...
fn handle_auto_click(app: &tauri::AppHandle, app_type: &AppType) -> Result<(), AppError> {
    if let Some(app_state) = app.try_state::<AppState>() {
        ...
        let p1_provider_id = queue
            ...
            .ok_or_else(|| AppError::Message(" Auto ".to_string()))?;
        ...
        if !is_running {
            ...
            if let Err(e) = futures::executor::block_on(proxy_service.start()) {
                ...
                return Err(AppError::Message(format!("failed: {e}")));
            }
        }

        log::info!("[Tray] Auto :  {app_type_str} ");
        if let Err(e) =
        ...
        if let Err(e) = app.emit("proxy-flags-changed", event_data.clone()) {
            log::error!(" proxy-flags-changed failed: {e}");
        }
        if let Err(e) = app.emit("provider-switched", event_data) {
            log::error!(" provider-switched failed: {e}");
        }
    }
    Ok(())
}
...
pub fn create_tray_menu(
    ...
    .map_err(|e| AppError::Message(format!("Lightweight Modefailed: {e}")))?;

    menu_builder = menu_builder.item(&lightweight_item).separator();

    let quit_item = MenuItem::with_id(app, "quit", tray_texts.quit, true, None::<&str>)
        .map_err(|e| AppError::Message(format!("Exitfailed: {e}")))?;
...
fn update_tray_usage_labels(app: &tauri::AppHandle) {
    ...
    for section in TRAY_SECTIONS.iter() {
        ...
        if let Err(e) = submenu.set_text(&new_label) {
            log::debug!("[Tray] {}failed: {e}", section.log_name);
        }
    }
}
...
pub fn apply_tray_policy(app: &tauri::AppHandle, dock_visible: bool) {
    ...
    if let Err(err) = app.set_dock_visibility(dock_visible) {
        log::warn!("failed to set Dock visibility status: {err}");
    }
    ...
    match event_id {
        ...
        "lightweight_mode" => {
            if crate::lightweight::is_lightweight_mode() {
                if let Err(e) = crate::lightweight::exit_lightweight_mode(app) {
                    log::error!("ExitLightweight Modefailed: {e}");
                }
            } else if let Err(e) = crate::lightweight::enter_lightweight_mode(app) {
                log::error!("Lightweight Modefailed: {e}");
            }
        }
        "quit" => {
            log::info!("Exit");
    ...
    for section in TRAY_SECTIONS.iter() {
        ...
        let current_id =
            match crate::settings::get_effective_current_provider(&app_state.db, &section.app_type)
            {
                ...
                Ok(None) => continue,
                Err(e) => {
                    log::warn!("[Tray] Read{log_name}failed: {e}");
                    continue;
                }
            };
            ...
            && (!is_official_provider || provider_uses_official_subscription(&current))
        {
            ...
            script_futures.push(async move {
                ...
                .await
                {
                    log::debug!("[Tray] {log_name} {provider_id} failed: {e}");
                }
            });
        }
    }
...
mod tests {
    ...
    fn tray_id_is_unique_to_app() {
        assert_eq!(TRAY_ID, "agent-switchboard");
