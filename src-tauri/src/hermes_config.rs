...
///
/// Input (from Agent Switchboard UI / database):
/// ```json
...
/// `providers:` dict. Agent Switchboard treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by Agent Switchboard)
...
/// keeping Agent Switchboard free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay Agent Switchboard must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a Agent Switchboard edit to an unrelated field would silently
...
// Agent Switchboard fills that gap by reading/writing the whole file as a markdown
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later Agent Switchboard edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a Agent Switchboard toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
