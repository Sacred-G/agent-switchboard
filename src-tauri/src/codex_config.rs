...
pub const CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME: &str = "agent-switchboard-model-catalog.json";
...
/// agent-switchboard–maintained catalog file referenced by `~/.codex/config.toml` and
...
/// agent-switchboard–generated file (identified by filename
/// `agent-switchboard-model-catalog.json`). A user-managed external catalog file is
...
/// Given `config.toml` text, resolve the on-disk path of the agent-switchboard–owned
...
fn set_codex_experimental_bearer_token(config_text: &str, token: &str) -> Result<String, AppError> {
    if config_text.trim().is_empty() {
        return Err(AppError::localized(
            ...
            "Codex Missing config.toml ConfigureWrite bearer token",
...
mod tests {
    ...
    fn unified_session_bucket_preserves_other_keys_and_explicit_routing() {
        let with_catalog = "model_catalog_json = \"agent-switchboard-model-catalog.json\"\n";
...
name = "any"
"#;
        let catalog_path = Path::new("/tmp/agent-switchboard-model-catalog.json");
    ...
    fn resolve_catalog_path_returns_none_when_config_missing_field() {
        let generated = PathBuf::from("/tmp/.codex/agent-switchboard-model-catalog.json");
    ...
    fn resolve_catalog_path_accepts_cc_switch_owned_file() {
        let generated = PathBuf::from("/tmp/.codex/agent-switchboard-model-catalog.json");
        let config = r#"model_catalog_json = "/tmp/.codex/agent-switchboard-model-catalog.json"
"#;
...
model = "glm-5"
"#;
        let regular_path = Path::new("/home/user/.codex/agent-switchboard-model-catalog.json");
    ...
    fn set_catalog_json_none_removes_cc_switch_owned_by_filename() {
        ...
        let input = r#"model_catalog_json = "/home/user/.codex/agent-switchboard-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove agent-switchboard-owned field regardless of path format"
        );
    }
...
"#;
        ...
        assert_eq!(
            result, None,
            "user-owned catalog should not be claimed by agent-switchboard"
        );
    }

    #[test]
    fn set_catalog_json_none_removes_relative_path() {
        let input = r#"model_catalog_json = "agent-switchboard-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove relative agent-switchboard-owned field"
        );
    }
}
