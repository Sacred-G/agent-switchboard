...
impl Database {
    ...
    pub(crate) fn apply_schema_migrations_on_conn(conn: &Connection) -> Result<(), AppError> {
        conn.execute("SAVEPOINT schema_migration;", [])
            .map_err(|e| AppError::Database(format!(" savepoint failed: {e}")))?;
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    0 => {
                        log::info!(" user_version=0 1");
                        Self::migrate_v0_to_v1(conn)?;
                        Self::set_user_version(conn, 1)?;
                    }
                    1 => {
                        log::info!(
                            "Migrate database from v1 to v2 (add usage stats and full fields, refactor skills table)"
                        );
                        Self::migrate_v1_to_v2(conn)?;
                        Self::set_user_version(conn, 2)?;
                    }
                    2 => {
                        log::info!("Migrate database from v2 to v3 (unified skills management architecture)");
                        Self::migrate_v2_to_v3(conn)?;
                        Self::set_user_version(conn, 3)?;
                    }
                    3 => {
                        log::info!("Migrate database from v3 to v4 (OpenCode support)");
                        Self::migrate_v3_to_v4(conn)?;
                        Self::set_user_version(conn, 4)?;
                    }
                    4 => {
                        log::info!("Migrate database from v5 to v6 (billing mode support)");
                        Self::migrate_v4_to_v5(conn)?;
                        Self::set_user_version(conn, 5)?;
                    }
                    5 => {
                        log::info!("Migrate database from v5 to v6 (usage rollup table + Copilot template type unified)");
                    ...
                    7 => {
                        log::info!("Migrate database from v7 to v8 (session log usage tracking + model pricing correction)");
                        Self::migrate_v7_to_v8(conn)?;
                        Self::set_user_version(conn, 8)?;
                    }
                    8 => {
                        log::info!("Migrate database from v8 to v9 (comprehensive model pricing supplement)");
                        Self::migrate_v8_to_v9(conn)?;
                        Self::set_user_version(conn, 9)?;
                    }
                    9 => {
                        log::info!("Migrate database from v9 to v10 (add Hermes Agent support)");
                        Self::migrate_v9_to_v10(conn)?;
                        Self::set_user_version(conn, 10)?;
                    }
                    10 => {
                        log::info!("Migrate database from v10 to v11 (usage_daily_rollups retains request_model dimension)");
    ...
    fn migrate_v1_to_v2(conn: &Connection) -> Result<(), AppError> {
        ...
        conn.execute("DROP INDEX IF EXISTS idx_failover_queue_order", [])
            .map_err(|e| AppError::Database(format!(" failover_queue failed: {e}")))?;
        ...
        .map_err(|e| AppError::Database(format!(" failover failed: {e}")))?;
    ...
    fn migrate_proxy_config_to_per_app(conn: &Connection) -> Result<(), AppError> {
        ...
        for (app, takeover, failover, retries, fb, idle, cb_f, cb_s, cb_t, cb_r, cb_m) in apps {
            ...
            ).map_err(|e| AppError::Database(format!(" {app} Configurefailed: {e}")))?;
        }
    ...
    fn migrate_skills_table(conn: &Connection) -> Result<(), AppError> {
        ...
        {
            log::info!("skills  v3  v1 -> v2 ");
            return Ok(());
        }

        if Self::has_column(conn, "skills", "app_type")? {
            log::info!("skills  app_type ");
            return Ok(());
        }
        ...
        .map_err(|e| AppError::Database(format!(" skills failed: {e}")))?;
        ...
        for (key, installed, installed_at) in old_skills {
            ...
            .map_err(|e| {
                AppError::Database(format!("failed to migrate skill {key} to new table: {e}"))
            })?;
        }
    ...
    fn migrate_v5_to_v6(conn: &Connection) -> Result<(), AppError> {
        ...
        .map_err(|e| AppError::Database(format!(" usage_daily_rollups failed: {e}")))?;
    ...
    fn migrate_v6_to_v7(conn: &Connection) -> Result<(), AppError> {
        ...
        log::info!("v6 -> v7 :  content_hash  updated_at ");
        Ok(())
    }
    ...
    fn migrate_v7_to_v8(conn: &Connection) -> Result<(), AppError> {
        ...
        .map_err(|e| AppError::Database(format!(" session_log_sync failed: {e}")))?;
        ...
        if Self::table_exists(conn, "model_pricing")? {
            ...
            for (model_id, input, output, cache_read, cache_creation) in pricing_fixes {
                ...
                .map_err(|e| AppError::Database(format!(" {model_id} failed: {e}")))?;
            }
        }

        log::info!("v7 -> v8 : data_source 、session_log_sync 、 13 ");
        Ok(())
    }
    ...
    fn migrate_v8_to_v9(conn: &Connection) -> Result<(), AppError> {
        ...
        .map_err(|e| AppError::Database(format!(" model_pricing failed: {e}")))?;
        conn.execute("DELETE FROM model_pricing", [])
            .map_err(|e| AppError::Database(format!("failed: {e}")))?;
        Self::seed_model_pricing(conn)?;
        log::info!("v8 -> v9 : ");
    ...
    pub(crate) fn set_user_version(conn: &Connection, version: i32) -> Result<(), AppError> {
        if version < 0 {
            return Err(AppError::Database("user_version ".to_string()));
        }
        let sql = format!("PRAGMA user_version = {version};");
        conn.execute(&sql, [])
            .map_err(|e| AppError::Database(format!("Write user_version failed: {e}")))?;
        Ok(())
    }
    ...
    fn validate_identifier(s: &str, kind: &str) -> Result<(), AppError> {
        if s.is_empty() {
            return Err(AppError::Database(format!("{kind} ")));
        }
    ...
    pub(crate) fn table_exists(conn: &Connection, table: &str) -> Result<bool, AppError> {
        ...
        while let Some(row) = rows.next().map_err(|e| AppError::Database(e.to_string()))? {
            let name: String = row
                .get(0)
                .map_err(|e| AppError::Database(format!("failed to parse table names: {e}")))?;
    ...
    fn add_column_if_missing(
        ...
        conn.execute(&sql, [])
            .map_err(|e| AppError::Database(format!(" {table}  {column} failed: {e}")))?;
