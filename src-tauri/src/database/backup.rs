...
const CC_SWITCH_SQL_EXPORT_HEADER: &str = "-- Agent Switchboard SQLite ";
...
impl Database {
    ...
    fn import_sql_string_inner(
        ...
        temp_conn
            ...
            .map_err(|e| AppError::Database(format!(" SQL failed: {e}")))?;
    ...
    fn validate_cc_switch_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        Err(AppError::localized(
            ...
            " Agent Switchboard  SQL 。",
            "Only SQL backups exported by Agent Switchboard are supported.",
        ))
    }
    ...
    fn restore_tables(
        ...
        for table in tables {
            ...
            target_conn
                ...
                .map_err(|e| AppError::Database(format!(" {table} failed: {e}")))?;
            ...
            let mut stmt = source_conn
                ...
                .map_err(|e| AppError::Database(format!("Read {table} failed: {e}")))?;
            let mut rows = stmt
                .query([])
                .map_err(|e| AppError::Database(format!(" {table} failed: {e}")))?;
    ...
    fn cleanup_db_backups(dir: &Path) -> Result<(), AppError> {
        ...
        for entry in sorted.into_iter().take(remove_count) {
            if let Err(err) = fs::remove_file(entry.path()) {
                log::warn!("failed {}: {}", entry.path().display(), err);
            }
        }
        Ok(())
    }
        ...
        match value {
            ...
            ValueRef::Text(t) => {
                let text = std::str::from_utf8(t)
                    .map_err(|e| AppError::Database(format!(" UTF-8: {e}")))?;
