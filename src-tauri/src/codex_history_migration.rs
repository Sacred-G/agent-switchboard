...
const LEGACY_CC_SWITCH_CODEX_MODEL_PROVIDER_ID: &str = "agent-switchboard";
...
fn restore_codex_state_db_official_threads(
    ...
    for chunk in ids.chunks(STATE_DB_ID_CHUNK) {
        ...
        changed += tx
            ...
            .map_err(|e| AppError::Database(format!(" Codex state DB provider failed: {e}")))?;
    }
    tx.commit()
        .map_err(|e| AppError::Database(format!(" Codex state DB failed: {e}")))?;
    Ok(changed)
...
fn backup_codex_state_db(
    ...
    backup
        ...
        .map_err(|e| AppError::Database(format!("Write Codex state DB failed: {e}")))?;
    Ok(())
}
...
"#
            ...
            Provider::with_id(
                "legacy-agent-switchboard".to_string(),
                "Legacy Agent Switchboard".to_string(),
                serde_json::json!({
                    "auth": {},
                    "config": r#"model_provider = "agent-switchboard"

[model_providers.agent-switchboard]
name = "AIHubMix"
base_url = "https://aihubmix.example/v1"
"#
                }),
...
"#
        ...
        assert_eq!(
            source_provider_ids,
            source_ids(&["aihubmix", "agent-switchboard", "rightcode"])
        );
        ...
        fs::write(
            ...
            concat!(
                ...
                "{\"type\":\"session_meta\",\"payload\":{\"id\":\"s3\",\"model_provider\":\"agent-switchboard\"}}\n",
        ...
        conn.execute_batch(
            ...
            INSERT INTO threads (id, model_provider) VALUES
                ...
                ('agent-switchboard-thread', 'agent-switchboard'),
        ...
        assert_eq!(
            ...
            source_ids(&["legacy-agent-switchboard", "rightcode"])
        );
        ...
        assert!(agent_switchboard_config
            .get("model_providers")
            .and_then(|value| value.get("agent-switchboard"))
            .is_none());
...
"#
    ...
    #[test]
    fn collects_legacy_agent_switchboard_provider_id_from_stored_config() {
        let db = Database::memory().expect("memory db");
        let mut provider = Provider::with_id(
            "generated-uuid".to_string(),
            ...
            serde_json::json!({
                "auth": {},
                "config": "model_provider = \"agent-switchboard\"\n\n[model_providers.agent-switchboard]\nname = \"AIHubMix\"\nbase_url = \"https://aihubmix.example/v1\""
            }),
            None,
        );
        provider.category = Some("aggregator".to_string());
        ...
        let ids = collect_source_model_provider_ids(&db).expect("collect ids");
        assert!(ids.contains("agent-switchboard"));
        assert!(ids.contains("aihubmix"));
        assert!(!ids.contains("generated-uuid"));
    }
...
"#
    ...
    #[test]
    fn migrates_legacy_agent_switchboard_provider_template_to_custom() {
        let db = Database::memory().expect("memory db");
        let provider = Provider::with_id(
            "legacy-agent-switchboard".to_string(),
            "Legacy Agent Switchboard".to_string(),
...
"#
        ...
        assert_eq!(
            ...
            vec!["legacy-agent-switchboard".to_string()]
        );

        let saved = db
            .get_provider_by_id("legacy-agent-switchboard", "codex")
            .expect("get provider")
            .expect("provider exists");
        let config_text = saved
            .settings_config
