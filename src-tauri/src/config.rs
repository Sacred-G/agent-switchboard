...
/// Migrates the old `.agent-switchboard` folder to `.agent-switchboard` if it exists.
pub fn migrate_cc_switch_dir() {
...
pub fn get_app_config_dir() -> PathBuf {
    ...
    let default_dir = get_home_dir().join(".agent-switchboard");

    #[cfg(windows)]
    {
        let default_db = default_dir.join("agent-switchboard.db");
        if !default_db.exists() {
            if let Ok(home_env) = std::env::var("HOME") {
                let trimmed = home_env.trim();
                if !trimmed.is_empty() {
                    let legacy_dir = PathBuf::from(trimmed).join(".agent-switchboard");
                    if legacy_dir.join("agent-switchboard.db").exists() {
                        log::info!(
...
pub fn read_json_file<T: for<'a> Deserialize<'a>>(path: &Path) -> Result<T, AppError> {
    if !path.exists() {
        return Err(AppError::Config(format!(": {}", path.display())));
    }
...
pub fn atomic_write(path: &Path, data: &[u8]) -> Result<(), AppError> {
    ...
    let parent = path
        .parent()
        .ok_or_else(|| AppError::Config("Invalid".to_string()))?;
    ...
    #[cfg(windows)]
    {
        ...
        fs::rename(&tmp, path).map_err(|e| AppError::IoContext {
            context: format!("failed: {} -> {}", tmp.display(), path.display()),
            source: e,
        })?;
    }
...
pub fn copy_file(from: &Path, to: &Path) -> Result<(), AppError> {
    fs::copy(from, to).map_err(|e| AppError::IoContext {
        context: format!("Copyfailed ({} -> {})", from.display(), to.display()),
        source: e,
    })?;
    Ok(())
}
