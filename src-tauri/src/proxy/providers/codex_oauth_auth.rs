...
/// User-Agent
const CODEX_USER_AGENT: &str = "agent-switchboard-codex-oauth";
...
impl CodexOAuthManager {
    ...
    pub async fn start_device_flow(&self) -> Result<GitHubDeviceCodeResponse, CodexOAuthError> {
        log::info!("[CodexOAuth]  Device Code ");
        ...
        log::info!(
            "[CodexOAuth]  Device Code Successuser_code: {}",
            device.user_code
        );
    ...
    pub async fn poll_for_token(
        ...
        log::debug!("[CodexOAuth]  Device Code");
        ...
        log::info!("[CodexOAuth]  OAuth Token");
        ...
        let refresh_token = tokens.refresh_token.clone().ok_or_else(|| {
            CodexOAuthError::TokenFetchfailed("Missing refresh_token".to_string())
        })?;
    ...
    pub async fn get_valid_token_for_account(
        ...
        log::info!("[CodexOAuth]  {account_id}  access_token ");
    ...
    pub async fn remove_account(&self, account_id: &str) -> Result<(), CodexOAuthError> {
        log::info!("[CodexOAuth] : {account_id}");
    ...
    fn write_store_atomic(&self, content: &str) -> Result<(), CodexOAuthError> {
        ...
        let parent = self
            .storage_path
            .parent()
            .ok_or_else(|| CodexOAuthError::IoError("Invalid".to_string()))?;
        let file_name = self
            .storage_path
            .file_name()
            .ok_or_else(|| CodexOAuthError::IoError("Invalid".to_string()))?
    ...
    fn load_from_disk_sync(&self) -> Result<(), CodexOAuthError> {
        ...
        if let Ok(mut accounts) = self.accounts.try_write() {
            *accounts = store.accounts;
            log::info!("[CodexOAuth]  {} accounts", accounts.len());
        }
