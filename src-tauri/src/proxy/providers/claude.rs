    ...
    fn extract_key(&self, provider: &Provider) -> Option<String> {
        if let Some(env) = provider.settings_config.get("env") {
            if let Some(key) = env
                ...
                .filter(|s| !s.is_empty())
            {
                log::debug!("[Claude]  ANTHROPIC_AUTH_TOKEN");
            ...
            if let Some(key) = env
                ...
                .filter(|s| !s.is_empty())
            {
                log::debug!("[Claude]  ANTHROPIC_API_KEY");
                return Some(key.to_string());
            }
            // OpenRouter key
            if let Some(key) = env
                ...
                .filter(|s| !s.is_empty())
            {
                log::debug!("[Claude]  OPENROUTER_API_KEY");
                return Some(key.to_string());
            }
            if let Some(key) = env
                .get("OPENAI_API_KEY")
                ...
                .filter(|s| !s.is_empty())
            {
                log::debug!("[Claude]  OPENAI_API_KEY");
                return Some(key.to_string());
            }
            // Gemini Native key
            if let Some(key) = env
                ...
                .filter(|s| !s.is_empty())
            {
                log::debug!("[Claude]  GEMINI_API_KEY");
                return Some(key.to_string());
            }
        }
        ...
        if let Some(key) = provider
            ...
            .filter(|s| !s.is_empty())
        {
            log::debug!("[Claude]  apiKey/api_key");
            return Some(key.to_string());
        }

        log::warn!("[Claude]  API Key");
        None
    }
...
impl ProviderAdapter for ClaudeAdapter {
    ...
    fn extract_base_url(&self, provider: &Provider) -> Result<String, ProxyError> {
        ...
        Err(ProxyError::ConfigError(
            "Claude Provider Missing base_url Configure".to_string(),
        ))
    }
    ...
    fn extract_auth(&self, provider: &Provider) -> Option<AuthInfo> {
        ...
        match provider_type {
            ProviderType::GeminiCli => {
                ...
                //
                // Agent Switchboard does not currently exchange the refresh_token for
            ...
            AuthStrategy::CodexOAuth => {
                vec![
                    ...
                    (
                        ...
                        HeaderValue::from_static("agent-switchboard"),
                    ),
                ]
            }
            AuthStrategy::GitHubCopilot => {
