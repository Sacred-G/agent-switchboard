...
impl CircuitBreaker {
    ...
    pub async fn record_success(&self, used_half_open_permit: bool) {
        ...
        if state == CircuitState::HalfOpen {
            ...
            if successes >= config.success_threshold {
                drop(config);
                log::info!(
                    "[{}] Circuit Broken HalfOpen → Closed (Normal)",
    ...
    pub async fn record_failure(&self, used_half_open_permit: bool) {
        ...
        match state {
            CircuitState::HalfOpen => {
                log::warn!(
                    "[{}] Circuit Broken HalfOpen failed → Open",
            ...
            CircuitState::Closed => {
                if failures >= config.failure_threshold {
                    log::warn!(
                        "[{}] Circuit Broken: failed {failures}  → Open",
    ...
    pub async fn reset(&self) {
        log::info!("[{}] Circuit Broken → Closed", log_cb::MANUAL_RESET);
        self.transition_to_closed().await;
    }
