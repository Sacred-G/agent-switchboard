...
pub(crate) async fn read_decoded_body(
    ...
    if let Some(encoding) = get_content_encoding(&headers) {
        log::debug!("[{tag}] : content-encoding={encoding}");
        match decompress_body(&encoding, &raw_bytes) {
            ...
            Ok(None) => {}
            Err(e) => {
                log::warn!("[{tag}] failed ({encoding}): {e}");
            }
        }
    }
...
pub async fn handle_streaming(
    ...
    if let Some(encoding) = get_content_encoding(response.headers()) {
        log::warn!(
            "[{}]  content-encoding={encoding}SSE Parsefailed。\
              accept-encoding  SSE 。",
            ctx.tag
        );
    }
...
pub async fn handle_non_streaming(
    ...
    if usage_logging_enabled(state) {
        ...
        log::debug!("[{}] usage logging Turn off usage Parse", ctx.tag);
    }
    ...
    fn drop(&mut self) {
        if let Some(collector) = self.collector.take() {
            if let Ok(handle) = tokio::runtime::Handle::try_current() {
                ...
                log::warn!("SSE  Tokio runtime  finish");
            }
        }
    }
}
...
async fn log_usage_internal(
    ...
    log::debug!(
        "[{app_type}] : id={request_id}, provider={provider_id}, model={model}, streaming={is_streaming}, status={status_code}, latency_ms={latency_ms}, first_token_ms={first_token_ms:?}, session={}, input={}, output={}, cache_read={}, cache_creation={}",
