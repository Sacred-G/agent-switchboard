...
pub(crate) const PROTOCOL_FORMAT: &str = "agent-switchboard-webdav-sync";
...
pub(crate) fn validate_manifest_compat(
    ...
    if manifest.version != PROTOCOL_VERSION {
        return Err(localized(
            ...
            format!(
                " manifest protocol: v{} ( v{PROTOCOL_VERSION})",
                manifest.version
            ),
            format!(
    ...
    match layout {
        RemoteLayout::Current if db_compat_version != DB_COMPAT_VERSION => {
            return Err(localized(
                ...
                format!(
                    ": db-v{db_compat_version} ( db-v{DB_COMPAT_VERSION})"
                ),
                format!(
...
pub(crate) fn validate_artifact_size_limit(artifact_name: &str, size: u64) -> Result<(), AppError> {
    if size > MAX_SYNC_ARTIFACT_BYTES {
        ...
        return Err(localized(
            ...
            format!("artifact {artifact_name} Exceeded{} MB", max_mb),
            format!(
...
pub(crate) fn verify_artifact(
    ...
    if bytes.len() as u64 != meta.size {
        return Err(localized(
            ...
            format!(
                "artifact {artifact_name}  (expected: {}, got: {})",
                meta.size,
                bytes.len(),
            ),
            format!(
    ...
    if actual_hash != meta.sha256 {
        return Err(localized(
            ...
            format!(
                "artifact {artifact_name} SHA256 failed (expected: {}..., got: {}...)",
    ...
    if let Err(db_err) = db.import_sql_string_for_sync(sql_str) {
        if let Err(rollback_err) = restore_skills_from_backup(&skills_backup) {
            return Err(localized(
                ...
                format!("failed: {db_err};  Skills failed: {rollback_err}"),
                format!(
...
mod tests {
    ...
    fn verify_artifact_rejects_size_mismatch() {
        ...
        assert!(
            err.to_string().contains("mismatch") || err.to_string().contains(""),
            "unexpected error: {err}"
        );
    }
