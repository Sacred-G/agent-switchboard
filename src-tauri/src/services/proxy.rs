...
impl ProxyService {
    ...
    fn claude_provider_with_effective_settings(
        ...
        .map_err(|e| format!(" claude Configurefailed: {e}"))?;
    ...
    pub async fn sync_codex_live_from_provider_while_proxy_active(
        ...
        .map_err(|e| format!(" codex Configurefailed: {e}"))?;
    ...
    fn get_current_provider_for_app(&self, app_type: &AppType) -> Result<Option<Provider>, String> {
        let Some(current_id) = crate::settings::get_effective_current_provider(&self.db, app_type)
            .map_err(|e| format!(" {app_type:?} failed: {e}"))?
        else {
            return Ok(None);
        };
        ...
        self.db
            ...
            .map_err(|e| format!("Read {app_type:?} failed: {e}"))
    }
    ...
    fn require_current_provider_for_app(&self, app_type: &AppType) -> Result<Provider, String> {
        self.get_current_provider_for_app(app_type)?
            .ok_or_else(|| format!("{app_type:?}  Live Configure"))
    }
    ...
    pub async fn start(&self) -> Result<ProxyServerInfo, String> {
        ...
        *self.server.write().await = Some(server);
        ...
        log::info!(": {}:{}", info.address, info.port);
        Ok(info)
    }
    ...
    pub async fn start_with_takeover(&self) -> Result<ProxyServerInfo, String> {
        ...
        if let Err(e) = self.sync_live_to_providers().await {
            if let Err(clean_err) = self.db.delete_all_live_backups().await {
                log::warn!(" Live failed: {clean_err}");
            }
            return Err(e);
        }
        ...
        if let Err(e) = self.takeover_live_configs().await {
            log::error!(" Live ConfigurefailedConfigure: {e}");
            match self.restore_live_configs().await {
                ...
                Err(restore_err) => {
                    log::error!("Configurefailed: {restore_err}");
                }
            }
            if started_proxy_before_takeover {
                let _ = self.stop().await;
    ...
    pub async fn set_takeover_for_app(&self, app_type: &str, enabled: bool) -> Result<(), String> {
        let app = AppType::from_str(app_type).map_err(|e| format!("Invalid: {e}"))?;
        ...
        if enabled {
            ...
            let current_config = self
                ...
                .await
                .map_err(|e| format!(" {app_type_str} Configurefailed: {e}"))?;
            ...
            if current_config.enabled {
                let has_backup = match self.db.get_live_backup(app_type_str).await {
                    Ok(v) => v.is_some(),
                    Err(e) => {
                        log::warn!("failed to read {app_type_str} backup (will continue rebuilding takeover): {e}");
                        false
                    }
                };
                let live_matches_current_proxy =
                    match self.live_takeover_matches_current_proxy(&app).await {
                        Ok(value) => value,
                        Err(e) => {
                            log::warn!(" {app_type_str} Configurefailed: {e}");
                            false
                        }
                    };
                ...
                log::warn!(
                    "{app_type_str}  backup={has_backup} live_matches_current_proxy={live_matches_current_proxy} Live"
                );
            }
            ...
            if let Err(e) = self.takeover_live_config_strict(&app).await {
                log::error!("{app_type_str}  Live Configurefailed: {e}");
        ...
        self.db
            ...
            .await
            .map_err(|e| format!(" {app_type_str} Live failed: {e}"))?;
        ...
        self.db
            .update_proxy_config_for_app(updated_config)
            .await
            .map_err(|e| format!(" {app_type_str} enabled failed: {e}"))?;

        self.db
            .clear_provider_health_for_app(app_type_str)
            .await
            .map_err(|e| format!(" {app_type_str} failed: {e}"))?;
    ...
    async fn sync_live_to_provider(&self, app_type: &AppType) -> Result<(), String> {
        let live_config = match app_type {
            ...
            _ => return Err("This application does not support proxy functionality".to_string()),
        };
                                ...
                                match env_obj {
                                    ...
                                    None => {
                                        ...
                                        if let Some(root) = provider.settings_config.as_object_mut()
                                        {
                                            ...
                                            log::warn!(
                                                "Claude provider settings_config Write Token (provider: {provider_id})"
                                            );
                                        }
                                    }
                                }
            ...
            AppType::Codex => {
                let provider_id =
                    crate::settings::get_effective_current_provider(&self.db, &AppType::Codex)
                        .map_err(|e| format!(" Codex failed: {e}"))?;
                            ...
                            .filter(|s| !s.is_empty() && *s != PROXY_TOKEN_PLACEHOLDER)
                        {
                                ...
                                .and_then(|v| v.as_object_mut())
                            {
                                ...
                                if let Some(root) = provider.settings_config.as_object_mut() {
                                    ...
                                    log::warn!(
                                        "Codex provider settings_config Write Token (provider: {provider_id})"
                                    );
                                }
                            }
                            ...
                            if let Err(e) = self.db.update_provider_settings_config(
                                ...
                                log::warn!("Sync Codex Token failed: {e}");
                            } else {
                                log::info!("Sync Codex Token  (provider: {provider_id})");
                            }
                        }
                    }
                }
            ...
            AppType::Gemini => {
                let provider_id =
                    crate::settings::get_effective_current_provider(&self.db, &AppType::Gemini)
                        .map_err(|e| format!(" Gemini failed: {e}"))?;
                            ...
                            .filter(|s| !s.is_empty() && *s != PROXY_TOKEN_PLACEHOLDER)
                        {
                                ...
                                .and_then(|v| v.as_object_mut())
                            {
                                ...
                                if let Some(root) = provider.settings_config.as_object_mut() {
                                    ...
                                    log::warn!(
                                        "Gemini provider settings_config Write Token (provider: {provider_id})"
                                    );
                                }
                            }
    ...
    async fn sync_live_to_providers(&self) -> Result<(), String> {
        ...
        log::info!("Live Configure Token Sync");
        Ok(())
    }
    ...
    pub async fn stop_with_restore(&self) -> Result<(), String> {
        if let Err(e) = self.stop().await {
            log::warn!("failed Live Configure: {e}");
        }
            ...
            if let Ok(mut config) = self.db.get_proxy_config_for_app(app_type).await {
                if config.enabled {
                    ...
                    if let Err(e) = self.db.update_proxy_config_for_app(config).await {
                        log::warn!(" {app_type} enabled failed: {e}");
                    }
                }
            }
        }
    ...
    async fn backup_live_configs(&self) -> Result<(), String> {
        ...
        if let Ok(config) = self.read_claude_live() {
            if Self::live_has_proxy_placeholder_for_app(&AppType::Claude, &config) {
                log::warn!("claude Live Configure;  stop  SSOT  Live");
            } else {
        ...
        if let Ok(config) = self.read_codex_live() {
            if Self::live_has_proxy_placeholder_for_app(&AppType::Codex, &config) {
                log::warn!("codex Live Configure;  stop  SSOT  Live");
            } else {
                let json_str = serde_json::to_string(&config)
                    .map_err(|e| format!(" Codex Configurefailed: {e}"))?;
                self.db
        ...
        if let Ok(config) = self.read_gemini_live() {
            if Self::live_has_proxy_placeholder_for_app(&AppType::Gemini, &config) {
                log::warn!("gemini Live Configure;  stop  SSOT  Live");
            } else {
                let json_str = serde_json::to_string(&config)
                    .map_err(|e| format!(" Gemini Configurefailed: {e}"))?;
                self.db
    ...
    async fn takeover_live_configs(&self) -> Result<(), String> {
        ...
        if let Ok(mut live_config) = self.read_claude_live() {
            ...
            log::info!("Claude Live Configure: {proxy_url}");
        }
        ...
        if let Ok(mut live_config) = self.read_codex_live() {
            ...
            log::info!("Codex Live Configure: {proxy_codex_base_url}");
        }
        ...
        if let Ok(mut live_config) = self.read_gemini_live() {
            ...
            log::info!("Gemini Live Configure: {proxy_url}");
        }
            ...
            AppType::Claude => {
                if let Ok(Some(backup)) = self.db.get_live_backup("claude").await {
                    let config: Value = serde_json::from_str(&backup.original_config)
                        .map_err(|e| format!("Parse Claude failed: {e}"))?;
                    self.write_claude_live(&config)?;
                    log::info!("Claude Live Configure");
                }
            }
            AppType::Codex => {
                if let Ok(Some(backup)) = self.db.get_live_backup("codex").await {
                    let config: Value = serde_json::from_str(&backup.original_config)
                        .map_err(|e| format!("Parse Codex failed: {e}"))?;
                    self.write_codex_live(&config)?;
                    log::info!("Codex Live Configure");
                }
            }
            AppType::Gemini => {
                if let Ok(Some(backup)) = self.db.get_live_backup("gemini").await {
                    let config: Value = serde_json::from_str(&backup.original_config)
                        .map_err(|e| format!("failed to parse Gemini backup: {e}"))?;
                    self.write_gemini_live(&config)?;
                    log::info!("Gemini Live Configure");
                }
            }
            _ => {}
        }
    ...
    async fn restore_live_config_for_app_with_fallback_inner(
        ...
        match self.restore_live_from_ssot_for_app(app_type) {
            Ok(true) => {
                log::info!("{app_type_str} Live Configure SSOT ");
                return Ok(());
            }
            Ok(false) => {
    ...
    fn write_live_config_for_app(&self, app_type: &AppType, config: &Value) -> Result<(), String> {
        match app_type {
            ...
            _ => Err("This application does not support proxy functionality".to_string()),
        }
    }
    ...
    fn restore_live_from_ssot_for_app(&self, app_type: &AppType) -> Result<bool, String> {
        ...
        write_live_with_common_config(self.db.as_ref(), app_type, provider)
            .map_err(|e| format!("Write {app_type:?} Live Configurefailed: {e}"))?;
    ...
    async fn update_live_backup_from_provider_inner(
        ...
        if matches!(app_type_enum, AppType::Codex) {
            let existing_backup_value = self
                ...
                .await
                .map_err(|e| format!("failed to read existing {app_type} backup: {e}"))?
                .map(|backup| {
                    serde_json::from_str::<Value>(&backup.original_config)
                        .map_err(|e| format!("failed to parse existing {app_type} backup: {e}"))
                })
                .transpose()?;
    ...
    pub(crate) async fn hot_switch_provider_inner(
        ...
        let app_type_enum =
            AppType::from_str(app_type).map_err(|_| format!("Invalid: {app_type}"))?;
        let provider = self
            .db
        ...
        let has_backup = self
            ...
            .await
            .map_err(|e| format!("failed to read {app_type} backup: {e}"))?
            .is_some();
        ...
        if has_backup && !live_taken_over && matches!(app_type_enum, AppType::Codex) {
            ...
            let auth = effective_settings
                .get("auth")
                .ok_or_else(|| "Codex Missing auth Configure".to_string())?;
            ...
            .map_err(|e| format!("Write Codex Configurefailed: {e}"))?;
        }
    ...
    fn preserve_codex_mcp_servers_from_existing_config(
        ...
        let target_obj = target_settings
            .as_object_mut()
            .ok_or_else(|| "Codex must be a JSON object".to_string())?;
        ...
        let mut target_doc = if target_config.trim().is_empty() {
            ...
            target_config
                ...
                .map_err(|e| format!("failed to parse new Codex config.toml: {e}"))?
        };
        ...
        let existing_doc = existing_config
            .parse::<toml_edit::DocumentMut>()
            .map_err(|e| format!("failed to parse existing Codex backup: {e}"))?;
    ...
    pub async fn switch_proxy_target(
        ...
        if outcome.logical_target_changed {
            log::info!(":  {app_type}  {provider_id}");
        } else {
            log::debug!(": {app_type}  {provider_id}");
        }
        Ok(())
    }
    ...
    fn read_claude_live(&self) -> Result<Value, String> {
        ...
        if !path.exists() {
            return Err("Claude Configure".to_string());
        }
        ...
        if !value.is_object() {
            ...
            return Err(format!(
                "Claude ConfigureError: must be a JSON object {kind}: {}",
                path.display()
            ));
        }
    ...
    fn write_claude_live(&self, config: &Value) -> Result<(), String> {
        ...
        write_json_file(&path, &settings).map_err(|e| format!("Write Claude Configurefailed: {e}"))
    }
    ...
    fn write_codex_live_for_provider(
        ...
        let auth = config
            .get("auth")
            .ok_or_else(|| "Codex ConfigureMissing auth ".to_string())?;
    ...
    fn read_gemini_live(&self) -> Result<Value, String> {
        ...
        if !env_path.exists() {
            return Err("Gemini .env ".to_string());
        }

        let env_map = read_gemini_env().map_err(|e| format!("Read Gemini env failed: {e}"))?;
    ...
    fn write_gemini_live(&self, config: &Value) -> Result<(), String> {
        ...
        let env_map = json_to_env(config).map_err(|e| format!(" Gemini Configurefailed: {e}"))?;
        write_gemini_env_atomic(&env_map).map_err(|e| format!("Write Gemini env failed: {e}"))?;
        Ok(())
    }
    ...
    pub async fn update_circuit_breaker_configs(
        ...
        if let Some(server) = self.server.read().await.as_ref() {
            ...
            log::info!("Circuit BrokenConfigure");
        } else {
            log::debug!("Circuit BrokenConfigure");
        }
        Ok(())
    }
    ...
    pub async fn update_circuit_breaker_config_for_app(
        ...
        if let Some(server) = self.server.read().await.as_ref() {
            server
                ...
                .await;
            log::info!(" {app_type} Circuit BrokenConfigure");
        } else {
            log::debug!("{app_type} Circuit BrokenConfigure");
        }
        Ok(())
    }
    ...
    pub async fn reset_provider_circuit_breaker(
        ...
        if let Some(server) = self.server.read().await.as_ref() {
            server
                ...
                .await;
            log::info!(" Provider {provider_id} (app: {app_type}) Circuit Broken");
        }
        Ok(())
    }
}
...
"#;
        ...
        assert!(
            catalog_path.exists(),
            "agent-switchboard-model-catalog.json must be created on provider switch"
        );
...
"#;
        ...
        assert!(
            message.contains("Write Codex Configurefailed") || message.contains("failed"),
    ...
    async fn codex_restore_from_backup_preserves_model_catalog_pointer() {
        ...
        // Pre-takeover Live state: config.toml points at the agent-switchboard generated
        ...
        assert!(
            ...
            "restored pointer must still reference the agent-switchboard generated catalog file"
        );
    }
    ...
    async fn codex_restore_from_backup_projects_inline_model_catalog() {
        ...
        assert!(
            catalog_path.exists(),
            "restore must generate the agent-switchboard catalog file on disk"
        );
    ...
    async fn codex_restore_empty_auth_backup_still_projects_inline_catalog() {
        ...
        assert!(
            ...
            "empty-auth restore must generate the agent-switchboard catalog file"
        );
        assert!(
