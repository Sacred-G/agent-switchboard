...
fn restore_live_settings_for_provider_backfill(
    ...
    // `modelCatalog` is a agent-switchboard–private field whose SSOT is the DB. Live's
...
pub fn sync_current_to_live(state: &AppState) -> Result<(), AppError> {
    ...
    for app_type in AppType::all() {
        if let Err(e) = crate::services::skill::SkillService::sync_to_app(&state.db, &app_type) {
            log::warn!("Sync Skill  {app_type:?} failed: {e}");
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        AppType::Codex => {
            ...
            // `modelCatalog` is a agent-switchboard private field that lives only in
            // the DB SSOT plus the `agent-switchboard-model-catalog.json` projection
        ...
        AppType::ClaudeDesktop => Err(AppError::localized(
            ...
            "Claude Desktop 3P Configure live Configure“ Claude ”。",
...
pub fn import_default_config(state: &AppState, app_type: AppType) -> Result<bool, AppError> {
        ...
        .detect_takeover_in_live_config_for_app(&app_type)
    {
        return Err(AppError::localized(
            ...
            "Live Configure。Turn off Live ConfigureRetry。",
    ...
    let settings_config = match app_type {
        ...
        AppType::ClaudeDesktop => {
            return Err(AppError::localized(
                ...
                "Claude Desktop 3P ConfigureRead“ Claude ”。",
    ...
    if let Some(config_value) = provider.settings_config.get("config") {
        if config_value.is_object() {
            ...
            return Err(AppError::localized(
                ...
                "Gemini ConfigureError: config must be null",
...
/// into the Agent Switchboard database. Each provider found will be added to the
