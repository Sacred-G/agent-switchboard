...
pub fn reapply_current_codex_official_live(state: &AppState) -> Result<bool, AppError> {
    ...
    if has_live_backup || live_taken_over {
        ...
        .map_err(|e| AppError::Message(format!(" Live failed: {e}")))?;
        return Ok(true);
    }
...
impl ProviderService {
    ...
    pub fn switch(state: &AppState, app_type: AppType, id: &str) -> Result<SwitchResult, AppError> {
        ...
        let _provider = providers
            .get(id)
            .ok_or_else(|| AppError::Message(format!(" {id} ")))?;
        ...
        if should_hot_switch && _provider.category.as_deref() == Some("official") {
            return Err(AppError::localized(
                ...
                "Cannot switch to official provider in proxy takeover mode; using proxy to access official API may result in account suspension。Turn off。",
        ...
        if should_hot_switch {
            ...
            .map_err(|e| AppError::Message(format!("Switch failed: {e}")))?;
            ...
            AppType::Claude => {
                if !provider.settings_config.is_object() {
                    return Err(AppError::localized(
                        ...
                        "Claude Configuremust be a JSON object",
            ...
            AppType::Codex => {
                let settings = provider.settings_config.as_object().ok_or_else(|| {
                    AppError::localized(
                        ...
                        "Codex Configuremust be a JSON object",
                ...
                let auth = settings.get("auth").ok_or_else(|| {
                    AppError::localized(
                        ...
                        format!(" {} Missing auth Configure", provider.id),
                ...
                if !auth.is_object() {
                    return Err(AppError::localized(
                        ...
                        format!(" {}  auth Configuremust be a JSON object", provider.id),
                        format!(
            ...
            AppType::OpenCode => {
                ...
                if !provider.settings_config.is_object() {
                    return Err(AppError::localized(
                        ...
                        "OpenCode Configuremust be a JSON object",
            ...
            AppType::OpenClaw => {
                ...
                if !provider.settings_config.is_object() {
                    return Err(AppError::localized(
                        ...
                        "OpenClaw Configuremust be a JSON object",
            ...
            AppType::Hermes => {
                ...
                if !provider.settings_config.is_object() {
                    return Err(AppError::localized(
                        ...
                        "Hermes Configuremust be a JSON object",
                ...
                let base_url = env
                    ...
                    .ok_or_else(|| {
                        AppError::localized(
                            ...
                            "Missing ANTHROPIC_BASE_URL Configure",
            ...
            AppType::Codex => {
                ...
                let base_url = if config_toml.contains("base_url") {
                    ...
                    return Err(AppError::localized(
                        ...
                        "config.toml Missing base_url Configure",
                ...
                let options = provider
                    ...
                    .ok_or_else(|| {
                        AppError::localized(
                            ...
                            "ConfigureError: Missing options",
