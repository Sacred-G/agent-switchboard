    ...
    let (manifest_bytes, etag) = s3::get_object(&creds, &manifest_key, MAX_MANIFEST_BYTES)
        ...
        .ok_or_else(|| {
            localized(
                ...
                "No synchronization data available to download from remote",
...
/// Example: `agent-switchboard-sync/v2/db-v6/default/manifest.json`
...
mod tests {
    ...
    fn s3_key_uses_v2_and_correct_format() {
        ...
        assert_eq!(key, "agent-switchboard-sync/v2/db-v6/default/manifest.json");
    }
    ...
    fn s3_key_matches_expected_pattern() {
        ...
        assert_eq!(parts[0], "agent-switchboard-sync");
