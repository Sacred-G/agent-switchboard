...
pub async fn put_bytes(
    ...
    .send()
    .await
    .map_err(|e| webdav_transport_error("webdav.put_failed", "PUT ", "PUT request", url, &e))?;
...
pub async fn get_bytes(
    ...
    .send()
    .await
    .map_err(|e| webdav_transport_error("webdav.get_failed", "GET ", "GET request", url, &e))?;
...
pub fn webdav_status_error(op: &str, status: StatusCode, url: &str) -> AppError {
    ...
    let mut zh = format!("WebDAV {op} failed: {status} ({safe_url})");
