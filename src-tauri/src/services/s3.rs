...
fn s3_transport_error(
    ...
    let (zh_reason, en_reason) = if err.is_timeout() {
        ...
        ("Request construction failed", "request build failed")
    } else {
...
fn s3_status_error(op: &str, status: StatusCode, url: &str) -> AppError {
    ...
    if matches!(status, StatusCode::UNAUTHORIZED | StatusCode::FORBIDDEN) {
        zh.push_str("。 Access Key ID  Secret Access Key。");
...
fn response_too_large_error(url: &str, max_bytes: usize) -> AppError {
    ...
    AppError::localized(
        ...
        format!("S3 Exceeded{} MB: {}", max_mb, redact_url(url)),
        format!(
...
pub(crate) async fn test_connection(creds: &S3Credentials) -> Result<(), AppError> {
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| s3_transport_error("s3.connection_failed", "Connect", "connection", &e))?;
...
pub(crate) async fn put_object(
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| s3_transport_error("s3.put_failed", "PUT ", "PUT request", &e))?;
...
pub(crate) async fn get_object(
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| s3_transport_error("s3.get_failed", "GET ", "GET request", &e))?;
...
pub(crate) async fn head_object(
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| s3_transport_error("s3.head_failed", "HEAD ", "HEAD request", &e))?;
...
mod integration_tests {
    ...
    async fn live_s3_put_get_head_roundtrip() {
        crate::proxy::http_client::init(None).ok();
        let creds = test_creds();
        let key = "agent-switchboard-sync/v2/default/_integration_test.json";
        ...
        // 404
        let r = get_object(&creds, "agent-switchboard-sync/_no_such_key", 1024).await;
        assert!(r.is_ok());
