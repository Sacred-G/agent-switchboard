...
pub const PROFILE_NAME: &str = "Agent Switchboard";
    ...
    let env = provider
        ...
        .ok_or_else(|| {
            AppError::localized(
                ...
                "Claude Desktop Missing env Configure",
    ...
    let base_url = env
        ...
        .ok_or_else(|| {
            AppError::localized(
                ...
                "Claude Desktop Missing ANTHROPIC_BASE_URL",
    ...
    let api_key = env
        ...
        .ok_or_else(|| {
            AppError::localized(
                ...
                "Claude Desktop Missing ANTHROPIC_AUTH_TOKENBearer Token",
...
pub fn validate_direct_provider(provider: &Provider) -> Result<(), AppError> {
    ...
    if !provider.settings_config.is_object() {
        return Err(AppError::localized(
            ...
            "Claude Desktop Configuremust be a JSON object",
    ...
    if let Some(meta) = provider.meta.as_ref() {
        if let Some(api_format) = meta.api_format.as_deref() {
            if !api_format.trim().is_empty() && api_format != "anthropic" {
                return Err(AppError::localized(
                    ...
                    "Claude Desktop  Anthropic Messages API",
        ...
        if matches!(
            ...
            return Err(AppError::localized(
                ...
                "This provider is in Claude Desktop local routing mode, cannot write in direct connection mode",
...
pub fn validate_proxy_provider(provider: &Provider) -> Result<(), AppError> {
    ...
    if let Some(meta) = provider.meta.as_ref() {
        if let Some(api_format) = meta.api_format.as_deref() {
            if !matches!(
                ...
                return Err(AppError::localized(
                    "claude_desktop.provider.api_format_unsupported",
                    format!("Claude Desktop  API : {api_format}"),
    ...
    if !has_proxy_base_url_and_key(provider) {
        return Err(AppError::localized(
            ...
            "Claude Desktop Missing Base URL  API Key",
    ...
    for (route_id, route) in routes {
        ...
        if !is_claude_safe_model_id(route_id) {
            return Err(AppError::localized(
                ...
                format!(
                    "Claude Desktop  claude-*  anthropic/claude-* : {route_id}"
                ),
                format!(
        ...
        if !upstream_model.is_empty() && upstream_model != route_id {
            return Err(AppError::localized(
                ...
                format!(
                    "Claude Desktop : {route_id} -> {upstream_model};  Claude "
                ),
                format!(
    ...
    let upstream_model = routes
        ...
        .ok_or_else(|| {
            AppError::localized(
                ...
                format!("Claude Desktop Configure: {requested_raw}"),
...
fn unsupported_platform_error() -> AppError {
    AppError::localized(
        ...
        " Claude Desktop 3P Configure。 macOS  Windows。",
