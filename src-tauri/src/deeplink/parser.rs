...
//!
//! Parses agent-switchboard:// URLs into DeepLinkImportRequest structures.
...
/// Parse a agent-switchboard:// URL into a DeepLinkImportRequest
///
/// Expected format:
/// agent-switchboard://v1/import?resource={type}&...
pub fn parse_deeplink_url(url_str: &str) -> Result<DeepLinkImportRequest, AppError> {
    ...
    if scheme != "agent-switchboard" {
        return Err(AppError::InvalidInput(format!(
            "Invalid scheme: expected 'agent-switchboard', got '{scheme}'"
        )));
    }
