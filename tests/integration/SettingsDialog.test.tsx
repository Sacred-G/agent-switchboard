...
describe("SettingsPage integration", () => {
  it("loads default settings from MSW", async () => {
    ...
    expect((appInput as HTMLInputElement).value).toBe("/home/mock/.agent-switchboard");
  });
  ...
  it("allows browsing and resetting directories", async () => {
    ...
    expect(appInput.value).toBe("/home/mock/.agent-switchboard");

    fireEvent.click(browseButtons[0]);
    await waitFor(() =>
      expect(appInput.value).toBe("/home/mock/.agent-switchboard/picked"),
    );

    fireEvent.click(resetButtons[0]);
    await waitFor(() => expect(appInput.value).toBe("/home/mock/.agent-switchboard"));
  ...
  it("notifies when export fails", async () => {
    ...
    expect(cancelMessage).toMatch(
      /settings\.selectFileFailed|Please choose a valid SQL backup file/,
    );
