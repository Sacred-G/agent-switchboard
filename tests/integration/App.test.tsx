...
vi.mock("@/components/providers/ProviderList", () => ({
  ProviderList: ({
    ...
    const currentProvider =
      providersArray.find((p: any) => p?.id === currentProviderId) ||
      providersArray[0] ||
      null;
    ...
    return (
      <div>
        ...
        <button onClick={() => currentProvider && onSwitch(currentProvider)}>
          switch
        </button>
        <button onClick={() => currentProvider && onEdit(currentProvider)}>edit</button>
        <button onClick={() => currentProvider && onDuplicate(currentProvider)}>
          duplicate
        </button>
        <button onClick={() => currentProvider && onConfigureUsage(currentProvider)}>
          usage
        </button>
  ...
  it("shows toast when duplicate cannot load live provider ids", async () => {
    ...
    await waitFor(() => {
      expect(toastErrorMock).toHaveBeenCalledWith(
        expect.stringContaining("Failed to read provider identifier from config"),
      );
    });
