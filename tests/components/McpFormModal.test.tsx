...
describe("McpFormModal", () => {
  ...
  it("应用预设后填充 ID 与Configure内容", async () => {
    renderForm();
    await waitFor(() =>
      expect(
  ...
  it("缺少Configure命令时阻止提交并提示错误", async () => {
    renderForm();
  ...
  it("支持向导生成Configure并自动填充 ID", async () => {
    renderForm();
...
`;
  ...
  it("编辑模式下保持 ID 并更新Configure", async () => {
  ...
  it("允许未选择任何应用保存Configure，并保持 apps 全 false", async () => {
    const { onSave } = renderForm();
  ...
  it("保存Failed时展示翻译后的错误并恢复按钮", async () => {
    const failingSave = vi.fn().mockRejectedValue(new Error("保存Failed"));
